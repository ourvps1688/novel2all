...
class TestRetrieverIntegration:
    @pytest.mark.asyncio
    async def test_retriever_initialized_lazily(
    ...
    @pytest.mark.asyncio
    async def test_search_relevant_events_returns_items(
        self, manager: MemoryManager
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_search_relevant_events_empty_corpus(
        self, manager: MemoryManager
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_load_for_writing_includes_events(
        self, manager: MemoryManager
    ) -> None:
        """load_for_writing 应把检索到的事件组装进 MemoryContext.events。"""
        # 先入库 5 条
        manager.retriever.add_events([
            {"chapter": i, "event_type": "timeline",
             "text": f"事件 {i}: 林雷相关情节"} for i in range(1, 6)
        ])
...
class TestSummarizerIntegration:
    @pytest.mark.asyncio
    async def test_load_recent_chapters_uncompressed(
        self, manager: MemoryManager
    ) -> None:
        ...
        # 5 章全量保留
        chapter_items = [i for i in items if i.source.endswith(tuple(f".{ch}" for ch in range(1, 6)))]
        assert len(chapter_items) >= 1

    @pytest.mark.asyncio
    async def test_load_recent_chapters_with_rolling(
        self, manager: MemoryManager
    ) -> None:
        """早期章节应按档位折叠成滚动摘要。"""
        tracker = manager.tracker
    ...
    @pytest.mark.asyncio
    async def test_load_recent_chapters_returns_memory_items(
        self, manager: MemoryManager
    ) -> None:
        tracker = manager.tracker
        state = tracker.read()
        ...
        assert len(non_empty) >= 1, f"所有 item 的 token_count 都 ≤ 0: {[i.token_count for i in items]}"


class TestUpdateAfterWritingIntegration:
    @pytest.mark.asyncio
    async def test_index_chapter_events_adds_to_retriever(
        self, manager: MemoryManager
    ) -> None:
...
class TestBackwardCompatibility:
    @pytest.mark.asyncio
    async def test_existing_memory_context_still_works(
        self, manager: MemoryManager
    ) -> None:
        ...
        ctx = await manager.load_for_writing(
            chapter=3, chapter_outline="继续", characters_involved=["林雷"],
        )
